...
impl TileCoordinates {
    ...
    pub fn new_find(x: i32, y: i32, my_id: Id, tile_map_name: &'static str, world: Arc<World<IsoData>>) -> TileCoordinates {
        let tile_id = {
            ...
            let id = tile_map.read().expect("Unable to Read Tile Map in New Find in Tile Coordinates").get_at(x, y).expect("Unable to Get At in New Find in Tile Coordinates");
            id
        };
    ...
    fn move_to(&mut self, id: Id, tile_map_name: &'static str, coords: (i32, i32), world: Arc<World<IsoData>>) {
        {
            ...
            tile.write().expect("Unable to Write Tile in Move To in Tile Coordinates").remove_from_tile(id);
        }
    ...
    fn finish_move(id: Id, tile_map_name: &'static str, coords: (i32, i32), world: Arc<World<IsoData>>) {
        ...
        let tile_id = tile_map.read().expect("Unable to Read Tile Map in Move To in Tile Coordinates").get_at_xy(coords).expect("Unable to Get At XY in Move To in Tile Coordinates");
        ...
        tile.write().expect("Unable to Write Tile in Move To in Tile Coordinates").add_to_tile(id);
    }
